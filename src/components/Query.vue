      ...
      <svg @click="$emit('remove')" width="1em" height="1em" viewBox="0 0 16 16" id="close" class="bi bi-x btn-outline-danger x-button" fill="currentColor" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="row mt-1 mb-1">
          ...
          <div v-if="selected.category==='athlete'" class="col-sm">
            <div class="form-group">
              <label>Athlete</label>
              <select class="form-select" v-model="selected.athlete">
                <option v-for="(ath, index) in athletes" :key="index" v-bind:value="ath.nid">{{ ath.field_last_name }}</option>
              </select>
            </div>
          </div>
          ...
          <div v-if="selected.category==='game'" class="col-sm">
            <div class="form-group">
              <label>Game</label>
              <select class="form-select" v-model="selected.game">
          ...
          <div class="col-sm">
            <div class="form-group">
              <label></label><br>
              <button id="submit" type="submit" class="btn btn-outline-primary" @click="submit(); show();">Submit</button>
            </div>
          </div>
        </div>
    </div>
...
export default {
  ...
  methods: {
    submit () {
      ...
      console.log('Do somethings after you clicked submit button.')
    },
    async getYears () {
    ...
    show () {
      console.log('Template button for showing some data.')
    }
  }
}
</script>
