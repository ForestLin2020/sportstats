  ...
  <div class="career-and-season">
    <div class="table-responsive">
      <table class="table-striped table-sm table-condensed table table-hover table-bordered total_up">
        <thead>
          <tr>
            <th colspan="8">Career & Season Stats</th>
          </tr>
          <tr>
            <th>Year</th>
            <th>GP</th>
        ...
        <tbody>
          <tr v-for="(totals,index) in totalsByYear" :key="index">
            ...
            <td>{{ totals.gp }}</td>
            <th>{{ totals.gs }}</th>
            <th>{{ totals.shots.g }}</th>
            <th>{{ totals.shots.a }}</th>
            <th>{{ totals.shots.sh }}</th>
            <th>{{ totals.shots.sog }}</th>
          ...
          <tr>
            ...
            <th v-if="careerTotals.shots && careerTotals.shots.g">{{ careerTotals.shots.g }}</th><th v-else>0</th>
            <th v-if="careerTotals.shots && careerTotals.shots.a">{{ careerTotals.shots.a }}</th><th v-else>0</th>
            <th v-if="careerTotals.shots && careerTotals.shots.sh">{{ careerTotals.shots.sh }}</th><th v-else>0</th>
            <th v-if="careerTotals.shots && careerTotals.shots.sog">{{ careerTotals.shots.sog }}</th><th v-else>0</th>
        ...
        <table class=" table-striped table-sm table-condensed table table-hover table-bordered total_up">
          ...
          <tbody>
            <tr v-for="(game, _id) in gamesFilterEventsByYear(year)" :key="_id">
              ...
              <td v-if="game.shots && game.shots.g" nowrap>{{ game.shots.g }}</td><td v-else>0</td>
              <td v-if="game.shots && game.shots.a" nowrap>{{ game.shots.a }}</td><td v-else>0</td>
              <td v-if="game.shots && game.shots.sh" nowrap>{{ game.shots.sh }}</td><td v-else>0</td>
              <td v-if="game.shots && game.shots.sog" nowrap>{{ game.shots.sog }}</td><td v-else>0</td>
    ...
    calTotalsByYear (years) {
      for (let i = 0; i < years.length; i++) {
        ...
        const gamesTotalByYear = {
          ...
          shots: {
            g: this.calTotal(gamesByYear, 'shots', 'g'),
            a: this.calTotal(gamesByYear, 'shots', 'a'),
            sh: this.calTotal(gamesByYear, 'shots', 'sh'),
            sog: this.calTotal(gamesByYear, 'shots', 'sog')
          },
          misc: {
    ...
    calCareerTotals () {
      if (this.totalsByYear) {
        this.careerTotals = {
          ...
          shots: {
            g: this.calTotal(this.totalsByYear, 'shots', 'g'),
            a: this.calTotal(this.totalsByYear, 'shots', 'a'),
            sh: this.calTotal(this.totalsByYear, 'shots', 'sh'),
            sog: this.calTotal(this.totalsByYear, 'shots', 'sog')
          },
          misc: {
